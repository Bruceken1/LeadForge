...
IMPORTANT: streaming=False for all models.
Groq does not reliably return tool_calls when streaming=True —
the response arrives as empty deltas, which LangChain assembles into
an empty AIMessage with no tool calls. This silently breaks all agent
handoffs and react-agent tool use. Use streaming=False always.
...
def get_llm(temperature: float = 0.3, model: str | None = None) -> ChatOpenAI:
    ...
    streaming=False is mandatory — Groq drops tool_call chunks when streaming,
    causing all agent handoffs and react-agent tool calls to silently return empty.
    """
    return ChatOpenAI(
        ...
        max_tokens      = 2048,
        streaming       = False,   # ← CRITICAL: must be False for tool calls to work on Groq
    )
