...
export default function AgentDashboard() {
  ...
  async function fetchRuns() {
    try {
      const r = await fetch(`${AGENT_API}/api/agent/runs?org_id=${org?.id}`);
  ...
  function startSSE(id: string) {
    ...
    const es = new EventSource(`${AGENT_API}/api/agent/stream/${id}`);
    sseRef.current = es;
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (payload.type === "status") {
          setStatus(payload.data?.status || payload.type as any);
        ...
        if (payload.type === "human_review_needed") {
  ...
  async function handleStart() {
    ...
    try {
      const r = await fetch(`${AGENT_API}/api/agent/run`, {
        method: "POST",
  ...
  async function handleStop() {
    if (!runId) return;
    await fetch(`${AGENT_API}/api/agent/run/${runId}`, { method: "DELETE" });
    sseRef.current?.close();
    setStatus("cancelled");
  ...
  async function handleApprove(approved: boolean) {
    ...
    await fetch(`${AGENT_API}/api/agent/approve`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="border-b border-border/50 px-6 py-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-primary">Autonomous SDR — powered by Vultr + Mistral</p>
        </div>
        {statusCfg && (
        ...
        <div className="w-80 border-r border-border/50 flex flex-col overflow-y-auto">
          ...
          <div className="p-5 border-t border-border/50">
            ...
            <p className="text-xs text-muted-foreground text-center mt-2">
              Powered by Mistral 7B via Vultr Serverless Inference
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div ref={eventLogRef} className="flex-1 overflow-y-auto p-4 space-y-2">
            ...
            {events.map((event, i) => {
              ...
              const Icon = cfg.icon;
              const isThinking = event.type === "message";
                ...
                <div key={i} className={`flex gap-3 text-sm animate-in fade-in-0 slide-in-from-bottom-2 duration-200`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className={`text-xs rounded-lg p-2.5 
                      ${event.type === "error" ? "bg-red-500/10 text-red-400 border border-red-500/20" :
                        event.type === "human_review_needed" ? "bg-orange-400/10 text-orange-400 border border-orange-400/20" :
              ...
              return (
                <div key={run.id}
                  className={`p-3 rounded-lg cursor-pointer transition-colors
                    ${run.id === runId ? "bg-primary/10 border border-primary/20" : "bg-muted/30 hover:bg-muted/50"}`}
                  onClick={() => {
                    setRunId(run.id);
